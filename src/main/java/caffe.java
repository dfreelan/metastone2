...
import net.demilich.metastone.bahaviour.ModifiedMCTS.ModifiedMCTS;
...
public class caffe {
        ...
        public int command() {
            ...
            for (int j = 0; j < iterations; j++) {
                ...
                for (int i = 0; i < layers.size(); i++) {
                    timer.Start();
                    // Although Reshape should bfe essentially free, we include it here
    ...
    public static void main(String[] args) {
        ...
        args = new String[2];
        args[0] = "train";
        args[1] = "-solver=/home/dfreelan/dev/networks/singleLayerExampleSolver.prototxt";
        
        flags.init(args);
        //GlobalInit(args);
        if (args.length > 0) {
            GetBrewFunction(args[0]).command();
            MCTSCritique blah = new MCTSCritique("");
        } else {
            ShowUsageWithFlags();
        }
    }
